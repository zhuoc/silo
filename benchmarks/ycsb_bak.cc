...
static unsigned g_txn_workload_mix[] = { 100, 0, 0, 0 };
...
public:
  ...
  txn_read()
  {
    ...
    try {
      ...
      if (likely(db->commit_txn(txn, temp_measurements))) {
		...
		temp_measurements->commit_txn += t_big.lap();
		measurements->add_measurement(temp_measurements, false);
	  }
    } catch (abstract_db::abstract_abort_exception &ex) {
      db->abort_txn(temp_measurements, txn);
    }
  ...
  txn_rmw()
  {
    ...
    try {
      ...
      ALWAYS_ASSERT(tbl->get(measurements, txn, u64_varkey(key).str(obj_key0), obj_v));
      computation_n += obj_v.size();
      tbl->put(measurements, txn, obj_key0, str().assign(YCSBRecordSize, 'c'));
  ...
  txn_scan()
  {
    ...
    try {
      tbl->scan(measurements, txn, kbegin, &kend, c);
      computation_n += c.n;
  ...
  for (size_t batchid = 0; batchid < nbatches;) {
    ...
    try {
      ...
      for (size_t i = batchid * batchsize + keystart; i < rend; i++) {
        ...
        tbl->insert(measurements, txn, k, v);
      }
